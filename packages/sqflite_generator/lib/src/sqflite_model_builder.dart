          ...
          (f) => f
            ...
            ..assignment = Code('''${entity.selectClassName}(${[
              for (final e in entity.aPs)
                if (e is AForeignKey)
                  '${e.nameDefault}: ${e.entityParent?.classType}Query.${entity.defaultSelectClass}'
                else
                  '${e.nameDefault}: true'
            ].join(',')})''')
      ...
      ..methods.addAll([
        Method((m) => m
          ...
          ..body = Code('''select?.\$check ==true?
            ...
            for (final e in entity.aPs)
              if (e is AForeignKey)
                '${e.entityParent?.classType}Query.\$createSelect(select?.${e.nameDefault}, ${e.subSelect(entity.foreignKeys.duplicated(e))})'
              else
        ...
        Method((m) => m
          ...
          ..body = Code('''
            ...
            for (final e in entity.aPs)
              if (e is AForeignKey)
                '${e.entityParent?.classType}Query.\$createWhere(where?.${e.nameDefault}, ${e.subSelect(entity.foreignKeys.duplicated(e))})'
              else
